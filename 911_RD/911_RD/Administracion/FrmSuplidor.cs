...
namespace _911_RD.Administracion
{
    public partial class FrmSuplidor : FrmBase
    {
        ...
        private void btn_TipoCliente_Click(object sender, EventArgs e)
        {
            try
            {
                using (FrmTipoSuplidor frmCargo = new FrmTipoSuplidor())
                {
        ...
        private void cargarTabla(string condicion)
        {
            try
            {
                using (TransporSysEntities db = new TransporSysEntities())
                {
                    ...
                    var empleado = from cli in db.SUPLIDORES
                                   join tCli in db.TIPOS_SUPLIDORES on cli.id_tipo_suplidor equals tCli.id_tipo_suplidor
                                   ...
                                   select new
                                   {
                                       ...
                                       id_cliente = cli.id_suplidor,
                                       id_tipo_cliente = cli.id_tipo_suplidor,
                                       tipo_cliente = tCli.tipo_suplidor,
                    ...
                    {
                        empleado = empleado.Where(em => em.Estado == true && (em.id_cliente.ToString().Contains(condicion) || em.Cedula.Contains(condicion)) || em.Nombre.Contains(condicion) || db.TIPOS_SUPLIDORES.FirstOrDefault(a => a.id_tipo_suplidor == em.id_tipo_cliente).descripcion.Contains(condicion));
                    }
        ...
        private void InsertarPuesto()
        {
            ...
            try
            {
                ...
                if (metodoscrud.crudSuplidor(tercero_t.ToString(), txt_id_tipo_cliente.Text.Trim().ToString(), cb_estado.SelectedIndex == 0 ? true : false) > 0)
